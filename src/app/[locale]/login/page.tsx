...
function LoginContent() {
  ...
  useEffect(() => {
    if (isInitialized && isAuthenticated) {
      const rawRedirect = searchParams.get('redirect') || '/play';
      router.push(stripLocalePrefix(rawRedirect));
    }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast.error(t('signUpFailed'), { description: error.message });
    } else {
      toast.success(t('accountCreated'), { description: t('checkEmail') });
    }
    setIsSubmitting(false);
  };
        ...
        <div className="absolute inset-0" style={{
          backgroundImage: `
            repeating-linear-gradient(
              ...
              var(--board-dark) 0px,
              var(--board-dark) 40px,
              var(--board-light) 40px,
              var(--board-light) 80px
            )
          `,
        }} />
      </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-center text-sm text-muted-foreground mt-6">
              {t('playWithoutAccount')}{' '}
              <Link href="/bot" className="text-primary hover:underline">
                {t('playVsBot')}
              </Link>
            </p>
          </CardContent>
